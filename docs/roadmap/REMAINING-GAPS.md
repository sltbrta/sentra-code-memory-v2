...
This is a pointer to the live, evidence-backed gap analysis rather than a
second copy that can drift. The authoritative remaining-work matrix is:
...
- `docs/decisions/0025-memory-session-lifecycle-parity.md` — the per-surface
  expose/defer/retire decision for memory, session, savings, and lifecycle.

When the audit is superseded by a newer one, update this pointer instead of
duplicating the matrix here.

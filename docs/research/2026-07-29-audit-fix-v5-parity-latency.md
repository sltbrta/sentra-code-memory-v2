...
- TUI: authority in `services.daemons`; rebind preserves; refuse placeholder OID
  on live socket.
- Web: upload size check before unbounded read; light Modal image env;
  product_client timeouts.
...
Light serve target: interactive tens of seconds cold, not 150s+. Bench still
multi-minute per-Q under agentic/map-reduce, but should no longer pad every arm
to 10s.
...
- **Cohere Rerank v3.5** preferred CE (then ZE, then lexical) — research: CE
  largest single gain
...
Still needed for ≥75 confidence: measure with gpt-5.4 (not Gemini) on a 40–80
slice before full500 judge.

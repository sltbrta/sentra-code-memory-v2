...
- **20-qid smoke** `erb-harden20-*` with gpt-5.4, **no official judge** (in
  flight)
- Full500 judge deferred until smoke shows model=gpt-5.4, cite_gold↑, pool_gold↑
  on hard set
...
3. 20-qid hard-set: pool@1 ≥70% of prior pool0, qualitative answer lift on
   wrong@gold samples
...
Completed hard set still often **pool@0** (0064/0074/0090). Easy goods
pool1+cite1.
...
**Still no** — need timeout rate &lt;10% and pool lift on hard 20 before
spending full500.
...
**Latency fix after 20b:** default self-consistency **off** (opt-in AUTO);
QUALITY max_synth_retry **1**.

### Full judge gate

Still open. Need timeout rate &lt;15% on hard 20 with single-shot synth, then
full500.
...
1. **ExpandLite** nested agentic reformulate/gap — HotLex+1 dense, no
   recovery/structure/grep
...
In flight: `erb-harden20c-latency-*` — gate for full500 = timeout &lt;15% and
warm p50 ≤90s.
...
- Full500 official judge deferred until hard-19 smoke confirms timeout&lt;15%
  and cite/pool lift
- residual pool@0 semantic may need careful dense-only recovery (not long HyDE
  HotLex)

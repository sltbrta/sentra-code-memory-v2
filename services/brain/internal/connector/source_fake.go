...
var fakeConnectorObservedAt = time.Date(2026, 8, 4, 12, 0, 0, 0, time.UTC)

func fakeConnectorDigest() string {
	return sha256Hex([]byte("ouroboros.fake-github-source.v1"))
}
...
func (f *FakeSourceAPI) Snapshot(ctx context.Context, owner, repo string) (SnapshotPage, error) {
	...
	return SnapshotPage{
		...
		ConnectorDigest: fakeConnectorDigest(),
		ObservedAt:      fakeConnectorObservedAt,
...
func (f *FakeSourceAPI) Delta(ctx context.Context, owner, repo, priorCursor string) (SnapshotPage, error) {
	...
	if !ok {
		return SnapshotPage{
			Cursor: priorCursor, Revision: "delta-" + priorCursor,
			ConnectorDigest: fakeConnectorDigest(), ObservedAt: fakeConnectorObservedAt,
			Objects: nil, Complete: true,
		}, nil
	}

...
func Handle(ctx context.Context, req Request) Response {
	...
	case VerbLifecycleInstall, VerbSessionProduct, VerbCodeDenseRerank,
		VerbHostedTenancy, VerbQueryAdvanced:
...
// --- Typed local agent-memory operators (issue #47) -----------------------
//
// These expose the bounded, local, projection-only memory.Store tier
// operations the standalone product owns. They open the store under dir
// (<dir>/memory/memory.json), identical to the memory_ask residual lane, but
// operate on the typed agent-memory tier rather than the company-doc store.

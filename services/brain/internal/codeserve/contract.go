...
func CatalogMetadata() []VerbSpec {
	return []VerbSpec{
		...
		{Name: string(VerbApplyChangeSet), Status: StatusStable, Surface: "jsonl",
			Summary:  "transactionally stage, verify, and promote an exact workflow ChangeSet. Writes files under root, so dispatch over MCP/HTTP requires the explicit operator-trust opt-in (issue #63).",
			Required: []string{"root", "changeset"}, Optional: []string{"index_cache", "workers"}, Aliases: []string{"apply-changeset"}, RequiresOperatorTrust: true},
